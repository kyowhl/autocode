...
var coverItems = [{value:1,text:"是"},{value:0,text:"否"}];
...
step2Panel = new FDFormPanel({
	domId:'step2Panel'
	,controls:[
	     new FDTextBox({domId:'txt-apiPath',name:'apiPath',nativeAttr:{placeholder:'G:\\workspacess_gyljr\\mqty_help_api'},width:'200px'})
	    ,new FDTextBox({domId:'txt-webPath',name:'webPath',nativeAttr:{placeholder:'G:\\workspacess_gyljr\\mqty_help_web'},width:'200px'})
	    ,new FDTextBox({domId:'txt-serverPath',name:'serverPath',nativeAttr:{placeholder:'G:\\workspacess_gyljr\\mqty_server_help'},width:'200px'})
	    ,new FDTextBox({domId:'txt-service',name:'service',defaultValue:'salse',nativeAttr:{placeholder:'salse'},width:'200px'})
		,new FDTextBox({domId:'txt-packageName',name:'packageName',defaultValue:'com.sanqing.api',width:'200px'}) 
		,new FDTextBox({domId:'txt-charset',name:'charset',defaultValue:'UTF-8',nativeAttr:{placeholder:'输入文件编码,如:UTF-8'},width:'100px'})
	    ,new FDRadio({domId:'rad-cover',name:'cover',defaultValue:1,items:coverItems})
	]
});
...
gridDS = new FDGrid({
	...
	,columns:[
 		...
 		,{text:'链接',name:'jdbcUrl',style:{width:'400px'}}
 		,{text:'用户/密码',style:{width:'150px'},name:'_account',render:function(row){
...
function selectDataSource(row){
	...
	testConnection(row,function(row){
		...
		step2Panel.getControl('service').setValue(row.username);
		step2Panel.getControl('packageName').setValue("com.sanqing.api."+row.username);
		showStep2(function(){		
